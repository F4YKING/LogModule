...
--[[ // Notes
	[10 September 2023]
	- Credit to Created by Pyseph#1015, Edited by F4YKING
...
return function(Table, IgnoredTables, DepthData, Path)
	...
	for Key, Value in next, Table do
		...
		if KeyClass == 'string' then
			...
			HasBrackets = true
			Key = '[' .. (KeyClass == 'table' and string.gsub(module.TableToString(Key, IgnoredTables, {Depth, Path}), '^%s*(.-)%s*$', '%1') or SerializeType(Key, KeyClass)) .. ']'
		end

		Value = ValueClass == 'table' and module.TableToString(Value, IgnoredTables, {Depth, Path}, Path .. (HasBrackets and '' or '.') .. Key) or SerializeType(Value, ValueClass)
